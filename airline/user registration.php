...
if ($uploadOk == 0) {
  // if everything is ok, try to upload file
   ...
   $query=mysql_query("SELECT * FROM `system_reg` where email='$email' ") or die(mysql_error());
	
	$duplicate=mysql_num_rows($query);
	
	if($duplicate==0){
		
		mysql_query("INSERT INTO `system_reg` set name='$name', u_name='$u_name', user='$user', pass='$pass', 
   mobile='$mobile', email='$email', dob='$dob', address1='$adress1', address2='$adress2', city='$city', state='$state',
   country='$country', photo='$target_file'") or die (mysql_error());
		
		header("location: user registration.php?msg=ok");
			
	} else{
		
		header("location: user registration.php?msg=fail");
		...
		<div class="col-md-2"></div>
			<div class="col-md-10">
				<div class="page-header">
					<h3 style="color:blue;">SYSTEM USER REGISTERATION FORM</h3>
				</div>
				<div class="row">
					<div class="col-md-7">
					<?php 
						...
						if($msg == 'ok'){
							...
							?>
						<center>
						<h2 class="alert alert-success">Sistem User Added Successfully !</h2>
						</center>
						<?php } if($msg == 'fail'){
						?>
						<center>
						...
						<form class="form-horizontal" method="POST" action="" enctype="multipart/form-data">
							...
							<div class="form-group">
								...
								<div class="col-md-8">
									<select class="form-control" name="user" id="select-option" required>
										<option value="Select City" disabled="disabled" selected="selected">User</option>
